let books = [{
    ...
    "bookId": 79,
    "title": "The Biography of Orwell's",
...
const booksEl = document.getElementById('books'); ///// 1

function createBookDiv(bookObject) { ///////////2
...
if (catButtonsEl) {
    catButtonsEl.addEventListener('click', function (event) {
        if (event.target.className == 'category-btn') {
            let selectedCet = event.target.innerText;
            let selectedBooks;
            if (selectedBooks !== 'ALL') {
                selectedBooks = filterBooksByCat(selectedCet)
            } else {
                selectedBooks == books
            }
            booksEl.innerHTML = '';
...
//////////////9
let myCart = cartItemsFromLocalStorage(); /////13
...
function showCartCount() {
    let meningKorzinamArr = cartItemsFromLocalStorage();
...
function cartItemsFromLocalStorage() {
    let result = JSON.parse(localStorage.getItem('meningKorzinam'));
...
function createCartItemTr(el) {
    ...
    trEl.className = ('cartItems__tr')
    trEl.innerHTML = `
 <td class="image"> <img src="img/${el.coverFileName}" alt="${el.title}"> </td>
 ...
 <td class="total_price">${el.price *1}</td>
 <td> <button type="button" class="delete-btn" data-id="${el.bookId}" >Delate</button> </td>`;
    return trEl;
}
...
if (cartItemsEl) {
    cartItemsEl.addEventListener('click', function (event) {
        if (event.target.className == 'delete-btn') {
            ...
            localStorage.setItem('meningKorzinam', JSON.stringify(meningKorzinamArr));

            let parentTr =  event.target.closest('tr');
          
            removeItemFromCart(parentTr)
            showCartCount();  
...
////17
function removeItemFromCart(trEl){
    cartItemsEl.removeChild(trEl);
...
///19
// let elementPos = meningKorzinam2.map(el => el.id).indexOf(39);
